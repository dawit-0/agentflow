...
async def insert_quick(db: aiosqlite.Connection, task_id: str, title: str,
    ...
    await db.execute(
        ...
        (task_id, title, prompt, model, work_dir, flow_id,
         ...
         retry_delay_seconds, sandbox, secrets_json),
    )
...
async def insert_spawned(db: aiosqlite.Connection, task_id: str, title: str,
                          ...
                          secrets_json: str = "[]") -> None:
    await db.execute(
        """INSERT INTO tasks (id, title, prompt, model, priority, work_dir, flow_id, permissions, agent_id, sandbox, secrets)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (task_id, title, prompt, model, priority, work_dir, flow_id, permissions_json, agent_id, sandbox, secrets_json),
    )
