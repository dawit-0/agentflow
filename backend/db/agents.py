...
async def insert(db: aiosqlite.Connection, agent_id: str, name: str,
                  ...
                  default_sandbox: str = "",
                  default_secret_names_json: str = "[]") -> None:
    await db.execute(
        """INSERT INTO agents (id, name, description, instructions, context, default_model, default_permissions, default_work_dir, default_flow_id, default_sandbox, default_secret_names)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (agent_id, name, description, instructions, context_json,
         default_model, permissions_json, default_work_dir, default_flow_id,
         default_sandbox, default_secret_names_json),
    )
